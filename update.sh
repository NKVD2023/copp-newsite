...
if [ -f "$APP_DIR/coppdb.sqlite" ]; then
    info "Создаю резервную копию базы данных перед обновлением..."
    cp "$APP_DIR/coppdb.sqlite" /tmp/coppdb_backup_deploy.sqlite
fi
...
if [ -f /tmp/coppdb_backup_deploy.sqlite ]; then
    info "Восстанавливаю рабочую базу данных..."
    mv /tmp/coppdb_backup_deploy.sqlite "$APP_DIR/coppdb.sqlite"
fi
